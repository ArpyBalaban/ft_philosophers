...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/13 20:31:27 by abalaban          #+#    #+#             */
/*   Updated: 2022/02/13 21:13:01 by abalaban         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	death_checker(t_rules *rules, t_philo *philo )
{
	...
	while (!(rules->already_ate))
	{
		...
		if (rules->dead)
			break ;
		death_checker_2(rules, philo);
	}
}

void	death_checker_2(t_rules *rules, t_philo *philo)
{
	int	i;

	i = 0;
	while (rules->nmbr_eat != -1 && i < rules->nmbr_philos \
			&& philo[i].ate >= rules->nmbr_eat)
			i++;
